...
_.extend(AssetVariant.prototype, {
    ...
    _getCacheHeaders: function() {
        ...
        if (this.cachedFilePath && now < this.expires) {
            var mtime = fs.statSync(this.cachedFilePath).mtime;
            d = moment(mtime);
        }

        if (!this._cacheHeaders || (now >= this.expires)) {
            var headers = {},
                cacheControl = [],
            ...
            //ensure the file is re-rendered when rebuilding the
            //cache headers by nulling the cachedFilePath
